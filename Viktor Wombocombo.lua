...
function OnUnload()
	PrintFloatText(myHero,2,"Viktor WomboCombo UnLoaded!")
end
function LoadMenu()
	Config = scriptConfig("Viktor WomboCombo", "Viktor WomboCombo")
	...
	PrintFloatText(myHero,2,"Viktor WomboCombo Loaded!")
end
...
function CastE(target)
	...
	if ValidTarget(target) then
		if GetDistance(target) <= rangeE and GetDistance(target)<=550 and EREADY then
			local EPos = tpE:GetPrediction(target)
			if EPos and GetDistance(EPos) then
				EnemyPos = {x = EPos.x, y = EPos.y, z = EPos.z}
				HeroPos = Vector(myHero.x, myHero.y, myHero.z)
				EMinRange = HeroPos +(HeroPos -EnemyPos)*(-50/GetDistance(EPos))
				Packet('S_CAST', { spellId = SPELL_3, fromX = EMinRange.x, fromY = EMinRange.z, toX = EPos.x, toY = EPos.z }):send()
			end
		elseif GetDistance(target) <= rangeE and EREADY then
			local EPos = tpE:GetPrediction(target)
			if EPos and GetDistance(EPos) then
				EnemyPos = {x = EPos.x, y = EPos.y, z = EPos.z}
				HeroPos = Vector(myHero.x, myHero.y, myHero.z)
				EMinRange = HeroPos +(HeroPos -EnemyPos)*(-550/GetDistance(EPos))
				Packet('S_CAST', { spellId = SPELL_3, fromX = EMinRange.x, fromY = EMinRange.z, toX = EPos.x, toY = EPos.z }):send()
			end
		end
	else
